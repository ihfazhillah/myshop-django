...
def backwards_func(apps, schema_editor):
    for model, fields in translation_fields.items():
        Model = apps.get_model('shop', model)
        ModelTranslation = apps.get_model('shop', '{}Translation'.format(model))
...
def _get_translation(obj, ModelTranslation):
    translations = ModelTranslation.objects.filter(master_id=obj.pk)
    try:
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('shop', '0004_add_translation_models'),
    ]
